...
void testcase() {
  ...
  // Add edges for parking with infinite capacity and cost 0 -> duration * P
  // since we shift
  for (int i = 0; i < nr_stations; ++i) {
    int previousTime;
    int previousIndex;
    for (auto it = indexMap[i].begin(); it != indexMap[i].end(); ++it) {
      ...
      eaG.addEdge(previousIndex, it->second, INT_MAX,  P);
      previousTime = it->first;
      previousIndex = it->second;
    }
  }

  // Add edges for all bookings with capacity 1 and cost profit -> P - profit
  // since we shift
  for (Booking b : bookings) {
    int duration = b.arrive - b.depart;
    eaG.addEdge(indexMap[b.from][b.depart], indexMap[b.to][b.arrive], 1,
                 P - b.profit);
  }
