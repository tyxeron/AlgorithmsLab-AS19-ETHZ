...
struct Info {
  ...
  bool operator==(const Info &other) {
    return (this->color == other.color) && (this->network == other.network) &&
...
bool try_two_color(const Triangulation &t, K::FT &squaredRange) {
  ...
  for (auto v = t.finite_vertices_begin(); v != t.finite_vertices_end(); ++v) {
    ...
    if (!(v->info().visited)) {
      ...
      while (!stack.empty()) {
        ...
        do {
          if (!t.is_infinite(c)) {
            if (CGAL::squared_distance(u->point(), c->point()) <=
                squaredRange) {
              if (c->info() == u->info()) {
                return false;
              }
  ...
  return !conflict_within_network(network0, squaredRange) &&
         !conflict_within_network(network1, squaredRange);
}
  ...
  for (int i = 0; i < nr_clues; ++i) {
    ...
    if (!colorable) {
      ...
      if (CGAL::squared_distance(holmes, watson) <= squaredRange) {
        ...
        if (CGAL::squared_distance(holmes, station_holmes->point()) >
                squaredRange ||
            CGAL::squared_distance(watson, station_watson->point()) >
                squaredRange) {
          // Unable to connect
          cout << "n";
        } else {
          if (station_holmes->info().network ==
              station_watson->info().network) {
            // In the same network
            cout << "y";
          } else {
